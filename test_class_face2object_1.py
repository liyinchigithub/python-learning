...
'''
    ...
    [方法重写]如果从父类继承的方法不能满足子类的需求，可以对其进行改写，这个过程叫方法的覆盖（override），也称为方法的重写。子类中重写父类的方法。
...
class people:
    ...
    def __init__(self,n,a,w):
        ...
        self.age = a
        self.__weight = w
        # 类中函数就可以使用selt.x引用构造函数中接收的变量，self代表类的实例，而非类
...
class student(people):
    ...
    def __init__(self,n,a,w,g):
        #  子类中，调用父类的构函
        people.__init__(self,n,a,w)# 必须在构造函数中调用父类的构函
        self.grade = g
    #覆写父类的方法
    def speak(self):
        print("%s 说: 我 %d 岁了，我在读 %d 年级"%(self.name,self.age,self.grade))
...
c = Child()          # 子类实例
c.myMethod()         # 子类调用重写方法，因为子类中有同名的方法，所以会调用子类中的方法
